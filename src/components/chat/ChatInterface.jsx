...
// ... (Sous-composants inchangés TypingIndicator, MessageTimer)
...
const MessageTimer = ({ createdAt, currentTick }) => {
  ...
  let timeLeftLabel = ""
  if (diff <= 0) { timeLeftLabel = "Expiré" } else {
...
export default function ChatInterface({ currentUser, targetUser, connection, onBack, onCreateConnection }) {
  ...
  // --- CRITIQUE : MARK AS READ ---
  // Dès qu'on est ici, on nettoie la DB. 
  // Le hook global verra le changement et enlèvera le point rouge partout.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    channelRef.current
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `connection_id=eq.${connection.id}` }, (payload) => {
          ...
          // Si je suis là et que je reçois un message, je le lis direct
  ...
  // ... (Fonctions d'édition/suppression inchangées, je raccourcis pour la lisibilité mais garde tout le reste)
  const handleDelete = async (msgId) => { /* ... code existant ... */ }
  const startEdit = (msg) => { setEditingId(msg.id); setEditText(msg.content) }
  const cancelEdit = () => { setEditingId(null); setEditText('') }
  const saveEdit = async () => { /* ... code existant ... */ }
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900 relative">
      <AnimatePresence>
        {errorToast && (
          <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} className="absolute bottom-20 left-1/2 transform -translate-x-1/2 z-50 bg-red-500/90 text-white px-4 py-2 rounded-full shadow-lg flex items-center gap-2 text-sm backdrop-blur-sm">
            <AlertCircle size={16} /> <span>{errorToast}</span>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="flex items-center gap-3 p-4 bg-slate-800 border-b border-white/10 shadow-md z-10">
        ...
        <div className="flex-1">
          ...
          <div className={`text-xs flex items-center gap-1 ${connection?.status === 'accepted' ? 'text-green-400' : 'text-gray-400'}`}>{statusLabel}</div>
        </div>
      </div>
      ...
      <div ref={containerRef} onScroll={handleScroll} className="flex-1 overflow-y-auto p-4 space-y-3 bg-black/20 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
        ...
        {messages.length === 0 && !hasMore && (
          <div className="text-center text-gray-500 text-sm mt-10 p-4">{!connection ? "Envoyer un message créera automatiquement une demande de lien." : "Aucun message."}</div>
        )}
          ...
          return (
            <div key={msg.id} className={`group flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
              <div className={`max-w-[85%] relative ${isEditing ? 'w-full' : ''}`}>
                ...
                {/* Boutons d'édition (cachés si pas moi) */}
                {isMe && !isEditing && (
      ...
      <div className="p-3 bg-slate-800 border-t border-white/10 flex gap-2 items-end relative">
        {showEmojiPicker && (
          <div className="absolute bottom-16 left-2 z-50 shadow-2xl rounded-2xl overflow-hidden border border-white/10">
            <EmojiPicker onEmojiClick={onEmojiClick} theme="dark" width={300} height={400} lazyLoadEmojis={true} />
          </div>
        )}
