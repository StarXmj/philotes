...
const MessageTimer = ({ createdAt, currentTick }) => {
  ...
  let timeLeftLabel = diff <= 0 ? "Expiré" : `${Math.floor(diff / (1000 * 60 * 60))}h ${Math.floor((diff % (1000 * 60 * 60)) / (1000 * 60))}m`
  return (<div className="text-[9px] opacity-60 flex items-center justify-end gap-1 mt-1 font-mono"><Clock size={10} /> {timeLeftLabel}</div>)
}
...
export default function ChatInterface({ currentUser, targetUser, connection, onBack, onCreateConnection }) {
  ...
  const showToast = (msg) => { setErrorToast(msg); setTimeout(() => setErrorToast(null), 4000) }

  // --- MISE A JOUR DB POUR PERSISTANCE (Si F5) ---
  useEffect(() => {
    const markAsRead = async () => {
        if (!connection?.id || !currentUser?.id) return
        await supabase.from('messages').update({ read_at: new Date().toISOString() }).eq('connection_id', connection.id).eq('receiver_id', currentUser.id).is('read_at', null)
    }
    markAsRead()
  ...
  const fetchMessages = async (offset = 0) => {
    ...
    try {
      const { data } = await supabase.from('messages').select('*').eq('connection_id', connection.id).order('created_at', { ascending: false }).range(offset, offset + PAGE_SIZE - 1)
      if (data.length < PAGE_SIZE) setHasMore(false)
      const newMessages = data.reverse()
      if (offset === 0) { setMessages(newMessages); setTimeout(() => scrollToBottom(), 100) } 
      else { setMessages(prev => [...newMessages, ...prev]) }
    } catch (err) { console.error(err) } finally { setIsLoadingMore(false) }
  }

  useEffect(() => {
    setMessages([]); setHasMore(true); fetchMessages(0)
    if (!connection?.id) return
    ...
    channelRef.current
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `connection_id=eq.${connection.id}` }, (payload) => {
          ...
          setIsRemoteTyping(false); setTimeout(() => scrollToBottom(), 100)
          // Lecture instantanée côté DB si chat ouvert
          if (payload.new.receiver_id === currentUser.id) { supabase.from('messages').update({ read_at: new Date().toISOString() }).eq('id', payload.new.id).then() }
      })
      ...
      .on('broadcast', { event: 'typing' }, (payload) => {
        if (payload.userId !== currentUser.id) {
          setIsRemoteTyping(true); clearTimeout(typingTimeoutRef.current); typingTimeoutRef.current = setTimeout(() => setIsRemoteTyping(false), 3000)
        }
      })
      .subscribe()
  ...
  const handleScroll = (e) => {
    ...
    if (scrollTop === 0 && hasMore && !isLoadingMore && messages.length >= PAGE_SIZE) {
      setIsLoadingMore(true); scrollHeightRef.current = scrollHeight; fetchMessages(messages.length)
    }
  }
  ...
  useLayoutEffect(() => {
    ...
    if (scrollHeightRef.current > 0 && containerRef.current) {
      const diff = containerRef.current.scrollHeight - scrollHeightRef.current
      containerRef.current.scrollTop = diff; scrollHeightRef.current = 0
    }
  }, [messages])
  ...
  const saveEdit = async () => {
    ...
    await supabase.from('messages').update({ content: editText }).eq('id', editingId)
  }
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900 relative">
      <AnimatePresence>
        {errorToast && (<motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} className="absolute bottom-20 left-1/2 transform -translate-x-1/2 z-50 bg-red-500/90 text-white px-4 py-2 rounded-full shadow-lg flex items-center gap-2 text-sm backdrop-blur-sm"><AlertCircle size={16} /> <span>{errorToast}</span></motion.div>)}
      </AnimatePresence>
