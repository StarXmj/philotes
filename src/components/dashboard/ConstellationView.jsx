...
const ConstellationView = ({ matches = [], myProfile, onSelectUser, unreadCounts = {}, myId }) => {
  ...
  const handleTouchMove = (e) => {
    if (e.touches.length === 2 && touchStartDist.current !== null) {
      ...
      setZoom(prev => Math.min(Math.max(0.2, prev + (d - touchStartDist.current) * 0.005), 3))
  ...
  const orbits = useMemo(() => {
    ...
    matches.forEach(m => {
        const gs = m.score_global || 0
        if (m.connection?.status === 'accepted') cat.orbit1.push(m) 
        else if (gs >= 85) cat.orbit2.push(m)
        else if (gs >= 60) cat.orbit3.push(m)
        else if (gs >= 45) cat.orbit4.push(m)
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full bg-slate-900/40 overflow-hidden cursor-grab active:cursor-grabbing group flex items-center justify-center touch-none" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
        <div className="absolute bottom-6 right-6 z-40 flex flex-col gap-2 bg-slate-800/80 p-2 rounded-xl"><button onClick={() => setZoom(Math.min(zoom + 0.2, 3))} className="p-2 text-white"><ZoomIn size={20}/></button><button onClick={() => setZoom(1)} className="p-2 text-white"><RotateCcw size={20}/></button><button onClick={() => setZoom(Math.max(zoom - 0.2, 0.2))} className="p-2 text-white"><ZoomOut size={20}/></button></div>
