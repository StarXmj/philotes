...
const MatchNode = memo(({ match, onClick, unreadCount, myId }) => {
    ...
    // Détection "Demande Reçue" (Status pending ET je suis le destinataire)
    const isPendingRequest = match.connection?.status === 'pending' && match.connection?.receiver_id === myId
    
    // Détection "Messages Non Lus"
    const hasUnreadMessages = unreadCount > 0
    
    const pct = Math.round(match.score * 100)
    
    // --- GESTION DES COULEURS & AURAS ---
    let borderColor = "border-white/20"
    let shadowClass = ""
    let animationClass = ""
    
    if (isPendingRequest) {
        // CAS 1 : DEMANDE D'AMI (Prioritaire) -> AURA VERTE PULSE
        borderColor = "border-green-400"
        shadowClass = "shadow-[0_0_20px_rgba(74,222,128,0.8)]" 
        ...
        shadowClass = "shadow-[0_0_15px_rgba(139,92,246,0.4)]"
    } 
    else if (pct >= 85) { 
        borderColor = "border-green-400"
        shadowClass = "shadow-[0_0_10px_rgba(74,222,128,0.3)]"
    }
...
const ConstellationView = ({ matches, myProfile, onSelectUser, unreadCounts = {}, myId }) => {
  ...
  const orbits = useMemo(() => {
    ...
    matches.forEach(m => {
        ...
        else if (percent >= 85) categorized.orbit2.push(m)
        else if (percent >= 70) categorized.orbit3.push(m)
        else if (percent >= 55) categorized.orbit4.push(m)
        else if (percent >= 40) categorized.orbit5.push(m)
        else categorized.orbit6.push(m)
    })
    return categorized
  }, [matches])
  ...
  return (
    ...
    >
        ...
        <div className="absolute top-4 left-4 z-40 px-3 py-1 bg-black/40 backdrop-blur-sm rounded-full border border-white/5 text-[10px] text-gray-400 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-2">
            <Move size={12}/> Glisser pour explorer • Pincer pour zoomer
        </div>

        <motion.div
            drag dragConstraints={containerRef} dragElastic={0.2} whileTap={{ cursor: "grabbing" }}
            onWheel={handleWheel} animate={{ scale: zoom }} transition={{ type: "spring", stiffness: 300, damping: 30 }}
        ...
        >
            <div className="relative w-[2400px] h-[2400px] flex items-center justify-center flex-shrink-0">
                ...
                {orbits.orbit6.length > 0 && <OrbitalRing radius={950} duration={240} reverse>{orbits.orbit6.map((m, i) => <Planet key={m.id} radius={950} angle={(i/orbits.orbit6.length)*2*Math.PI} duration={240} reverse><MatchNode match={m} onClick={onSelectUser} unreadCount={unreadCounts[m.id] || 0} myId={myId} /></Planet>)}</OrbitalRing>}
            </div>
        </motion.div>
    </div>
  )
