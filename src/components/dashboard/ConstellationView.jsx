...
const MatchNode = memo(({ match, onClick, unreadCount, myId }) => {
    ...
    // --- COULEURS DES CERCLES (Selon tes paliers) ---
...
const ConstellationView = ({ matches = [], myProfile, onSelectUser, unreadCounts = {}, myId }) => {
  ...
  const [zoom, setZoom] = useState(0.6) // Zoom un peu plus large par défaut pour voir tout le monde
  const touchStartDist = useRef(null)

  // Gestion du zoom (inchangée)
  ...
  // --- LOGIQUE DES 5 ORBITES ---
  const orbits = useMemo(() => {
    const cat = { orbit1: [], orbit2: [], orbit3: [], orbit4: [], orbit5: [] }
    if (!matches || !Array.isArray(matches)) return cat;
    
    matches.forEach(m => {
        const gs = m.score_global || m.score * 100 || 0
        const isFriend = m.connection?.status === 'accepted'

        if (isFriend) {
            cat.orbit1.push(m) // Orbite 1 : Amis
        } else if (gs >= 85) {
            cat.orbit2.push(m) // Orbite 2 : 85-100%
        } else if (gs >= 60) {
            cat.orbit3.push(m) // Orbite 3 : 60-85%
        } else if (gs >= 45) {
            cat.orbit4.push(m) // Orbite 4 : 45-60%
        } else {
            cat.orbit5.push(m) // Orbite 5 : < 45%
        }
    })
    return cat
  }, [matches])
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full bg-slate-900/40 overflow-hidden cursor-grab active:cursor-grabbing group flex items-center justify-center touch-none" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
        ...
        <motion.div drag dragConstraints={containerRef} whileTap={{ cursor: "grabbing" }} onWheel={handleWheel} animate={{ scale: zoom }} className="flex items-center justify-center origin-center">
            {/* On agrandit la zone pour faire tenir tout le monde */}
            <div className="relative w-[3000px] h-[3000px] flex items-center justify-center flex-shrink-0">
                ...
                {/* Rendu des 5 orbites avec rayons ajustés */}
                {orbits.orbit1.length > 0 && <OrbitalRing radius={250} duration={60}>{orbits.orbit1.map((m, i) => <Planet key={m.id} radius={250} angle={(i/orbits.orbit1.length)*2*Math.PI} duration={60}><MatchNode match={m} onClick={onSelectUser} unreadCount={unreadCounts[m.id] || 0} myId={myId} /></Planet>)}</OrbitalRing>}
                {orbits.orbit2.length > 0 && <OrbitalRing radius={450} duration={90} reverse>{orbits.orbit2.map((m, i) => <Planet key={m.id} radius={450} angle={(i/orbits.orbit2.length)*2*Math.PI} duration={90} reverse><MatchNode match={m} onClick={onSelectUser} unreadCount={unreadCounts[m.id] || 0} myId={myId} /></Planet>)}</OrbitalRing>}
                {orbits.orbit3.length > 0 && <OrbitalRing radius={650} duration={120}>{orbits.orbit3.map((m, i) => <Planet key={m.id} radius={650} angle={(i/orbits.orbit3.length)*2*Math.PI} duration={120}><MatchNode match={m} onClick={onSelectUser} unreadCount={unreadCounts[m.id] || 0} myId={myId} /></Planet>)}</OrbitalRing>}
                {orbits.orbit4.length > 0 && <OrbitalRing radius={850} duration={150} reverse>{orbits.orbit4.map((m, i) => <Planet key={m.id} radius={850} angle={(i/orbits.orbit4.length)*2*Math.PI} duration={150} reverse><MatchNode match={m} onClick={onSelectUser} unreadCount={unreadCounts[m.id] || 0} myId={myId} /></Planet>)}</OrbitalRing>}
                {orbits.orbit5.length > 0 && <OrbitalRing radius={1100} duration={200}>{orbits.orbit5.map((m, i) => <Planet key={m.id} radius={1100} angle={(i/orbits.orbit5.length)*2*Math.PI} duration={200}><MatchNode match={m} onClick={onSelectUser} unreadCount={unreadCounts[m.id] || 0} myId={myId} /></Planet>)}</OrbitalRing>}
            </div>
        </motion.div>
    </div>
  )
