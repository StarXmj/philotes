...
export default function Profile() {
  ...
  const handleSaveProfile = async (e) => {
    ...
    try {
        ...
        if (realPhotoFile) {
            ...
            finalPrivateUrl = publicUrl; oldPhotoUrlRef.current = publicUrl
        } else if (currentRealPhotoUrl === null && oldPhotoUrlRef.current) {
            await deleteOldAvatar(); finalPrivateUrl = null; oldPhotoUrlRef.current = null
        }
        const { error } = await supabase.from('profiles').update({
            pseudo, sexe, type_diplome: selectedEtude, domaine: selectedTheme, annee_etude: selectedAnnee, intitule: selectedNom, parcours: selectedParcours || null, etudes_lieu: selectedLieu, avatar_public: selectedPublicAvatar, avatar_prive: finalPrivateUrl
        }).eq('id', user.id)
        ...
        setHasChanges(false); alert("Profil mis à jour avec succès !")
  ...
  const handleUpdatePassword = async (e) => {
    ...
    else { setPassMessage({ type: 'success', text: "Mot de passe mis à jour !" }); setIsEditingPassword(false); setPasswordData({ new: '', confirm: '' }) }
  ...
  return (
    <div className="min-h-screen bg-philo-dark p-6 md:p-10 text-white pb-40">
      <div className="max-w-3xl mx-auto space-y-8">
        ...
        <form id="main-profile-form" onSubmit={handleSaveProfile} className="space-y-8">
                ...
                <div>
                    ...
                    <div className="flex gap-2 items-stretch">
                        <label className={`flex-1 flex items-center gap-4 cursor-pointer group bg-black/20 p-3 rounded-xl border border-white/5 hover:bg-black/40 hover:border-white/20 transition`}>
                            ...
                            <div className="flex-1"><div className="text-white font-medium text-sm">{realPhotoPreview || currentRealPhotoUrl ? "Changer" : "Ajouter"}</div><div className="text-gray-500 text-xs">Visible uniquement par amis</div></div>
                            <input type="file" accept="image/*" onChange={handlePhotoUpload} className="hidden" />
                        </label>
                        {(realPhotoPreview || currentRealPhotoUrl) && (<button type="button" onClick={handleRemovePhoto} className="px-4 flex flex-col items-center justify-center bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 rounded-xl text-red-400 transition"><Trash2 size={20} /></button>)}
                    </div>
                </div>
            </div>
          ...
          <div className="bg-white/5 p-6 rounded-3xl border border-white/10 shadow-lg">
            ...
            <div className="space-y-4">
               <div><label className="text-xs text-gray-400 block mb-1">Diplôme</label><select required value={selectedEtude} onChange={e => { setSelectedEtude(e.target.value); setSelectedTheme(''); markChanged(); }} className={selectStyle}>{etudesOpts.map(o => <option key={o} value={o}>{o}</option>)}</select></div>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 <div><label className="text-xs text-gray-400 block mb-1">Domaine</label><select required value={selectedTheme} onChange={e => { setSelectedTheme(e.target.value); setSelectedAnnee(''); markChanged(); }} className={selectStyle}>{themesOpts.map(o => <option key={o} value={o}>{o}</option>)}</select></div>
                 {!selectedEtude.includes('Doctora') && (<div><label className="text-xs text-gray-400 block mb-1">Année</label><select required value={selectedAnnee} onChange={e => { setSelectedAnnee(e.target.value); setSelectedNom(''); markChanged(); }} className={selectStyle}>{anneesOpts.map(o => <option key={o} value={o}>{o}</option>)}</select></div>)}
               </div>
               <div><label className="text-xs text-gray-400 block mb-1">{selectedEtude.includes('Doctora') ? "Matière / Sujet" : "Intitulé Exact"}</label>{selectedEtude.includes('Doctora') ? (<input type="text" value={selectedNom} onChange={e => { setSelectedNom(e.target.value); markChanged(); }} className={selectStyle} />) : (<select required value={selectedNom} onChange={e => { setSelectedNom(e.target.value); setSelectedParcours(''); markChanged(); }} className={selectStyle}>{nomsOpts.map(o => <option key={o} value={o}>{o}</option>)}</select>)}</div>
               {(parcoursOpts.length > 0 || selectedEtude.includes('Doctora')) && (<div><label className="text-xs text-gray-400 block mb-1">{selectedEtude.includes('Doctora') ? "Nom de la thèse" : "Option / Parcours"}</label>{selectedEtude.includes('Doctora') ? (<input type="text" value={selectedParcours} onChange={e => { setSelectedParcours(e.target.value); markChanged(); }} className={selectStyle} />) : (<select value={selectedParcours} onChange={e => { setSelectedParcours(e.target.value); markChanged(); }} className={selectStyle}><option value="">(Aucune)</option>{parcoursOpts.map(o => <option key={o} value={o}>{o}</option>)}</select>)}</div>)}
               <div><label className="text-xs text-gray-400 block mb-1 flex items-center gap-1"><MapPin size={12}/> Campus</label><select required value={selectedLieu} onChange={e => { setSelectedLieu(e.target.value); markChanged(); }} className={selectStyle}>{CAMPUS_LIST.map(o => <option key={o} value={o}>{o}</option>)}</select></div>
            </div>
          </div>
        </form>

        {/* VIBES (CODE IDENTIQUE) */}
        <div className="bg-gradient-to-br from-purple-900/40 to-blue-900/40 p-6 rounded-3xl border border-white/10 shadow-lg">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6"><div className="flex items-center gap-2 text-purple-300"><BrainCircuit size={24} /> <h2 className="font-bold uppercase text-sm tracking-wider">Ma Personnalité</h2></div><button type="button" onClick={handleRetakeQuiz} className="px-4 py-2 bg-purple-500/20 hover:bg-purple-500/30 border border-purple-500/50 rounded-xl text-xs font-bold text-purple-200 transition flex items-center gap-2"><Sparkles size={14} /> Refaire le test</button></div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">{myVibes.map((vibe, idx) => (<div key={idx} className="bg-black/30 border border-white/10 rounded-xl p-3"><p className="text-[10px] uppercase text-philo-primary font-bold mb-1 opacity-70">{vibe.question}</p><p className="text-sm font-medium text-white">{vibe.answer}</p></div>))}</div>
        </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-6 shadow-lg">
            ...
            {!isEditingPassword ? (
                ...
                <form onSubmit={handleUpdatePassword} className="bg-black/20 p-4 rounded-xl border border-white/10 space-y-4">
                    ...
                    <div className="relative"><Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" size={16} /><input type={showPassword ? "text" : "password"} placeholder="Nouveau mot de passe" value={passwordData.new} onChange={(e) => setPasswordData({...passwordData, new: e.target.value})} className="w-full bg-black/40 border border-white/10 rounded-lg py-3 pl-10 pr-10 text-sm text-white focus:border-philo-primary outline-none transition" /><button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-white">{showPassword ? <EyeOff size={16}/> : <Eye size={16}/>}</button></div>
                    <div className="space-y-1"><div className="flex justify-between text-[10px] text-gray-400 px-1"><span>Force</span><span className={strength >= 3 ? "text-green-400" : "text-gray-400"}>{getStrengthLabel()}</span></div><div className="h-1.5 w-full bg-black/40 rounded-full overflow-hidden"><motion.div className={`h-full ${getStrengthColor()}`} initial={{ width: 0 }} animate={{ width: `${(strength / 4) * 100}%` }} transition={{ duration: 0.3 }} /></div></div>
                    <div className="relative"><Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" size={16} /><input type={showPassword ? "text" : "password"} placeholder="Confirmer le mot de passe" value={passwordData.confirm} onChange={(e) => setPasswordData({...passwordData, confirm: e.target.value})} className="w-full bg-black/40 border border-white/10 rounded-lg py-3 pl-10 pr-10 text-sm text-white focus:border-philo-primary outline-none transition" /></div>
        ...
        {/* ACTION BAR (SAVE + LOGOUT) */}
        <div className="sticky bottom-6 pt-4 space-y-3">
           <div className="absolute inset-0 bg-gradient-to-t from-philo-dark via-philo-dark to-transparent -z-10 h-32 -top-16" />
